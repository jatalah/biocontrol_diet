# An investigation into the seasonality and quality of food on marina pontoon structures for the candidate marine biological control agent, *Lunella smaragda* (Gastropoda)

## Authors

Lauren M. Fletcher<sup>a,\*</sup>, Grant A. Hopkins<sup>a,\*</sup>, Matthew R. Miller<sup>a</sup>, Natalí J. Delorme<sup>a</sup>, Rebecca M. McMullin<sup>a</sup>, Lizenn Delisle<sup>a,b</sup> and Javier Atalah<sup>a</sup>

a Cawthron Institute, Private Bag 2, Nelson 7042, New Zealand  
b Sorbonne Université, CNRS, UMR7144 AD2M, ECOPHY, Station Biologique de Roscoff, 29680 Roscoff, France  

\* These authors contributed equally and should be regarded as joint co-authors.

---

## Overview

This repository contains data, analysis scripts, and supporting material for the study:

> *An investigation into the seasonality and quality of food on marina pontoon structures for the candidate marine biological control agent,* *Lunella smaragda* *(Gastropoda)*

The study investigates temporal variation in the quantity and nutritional quality of biofouling communities growing on marina pontoon structures and evaluates their suitability as a food resource for the endemic New Zealand marine gastropod *Lunella smaragda*. The project contributes to the development of environmentally sustainable marine biofouling management strategies using native grazing species as biological control agents.

---

## Research Objectives

The main objectives of this study were to:

- Characterise the nutritional availability (i.e. quantity) and quality of biofilm and biofouling communities growing on marina pontoons across different stages of community development.
- Assess seasonal variation in food resources over periods relevant to marine biocontrol applications, including warmer months with high fouling growth and cooler months with lower growth rates.
- Evaluate signs of food limitation stress in *Lunella smaragda* under different food supply scenarios using survival, weight change, biochemical body composition, and malondialdehyde (MDA) levels.
- Improve understanding of the relationship between the ecological requirements of *Lunella smaragda* and the temporal dynamics of marina fouling communities.
- Inform optimal stocking densities and future operational deployment of *Lunella smaragda* as a marine biofouling biocontrol agent.

---

## Keywords

- Biofouling suppression
- Control tools
- Grazing pressure
- Marine biosecurity management
- Stocking densities
- Stress

---

## Repository Structure

```text
├── data/                # Raw and processed datasets
├── figures/             # R scripts for analyses and figures
├── others/             # Figures, tables, and model outputs
├── tables/                # Supplementary material and manuscripts
└── README.md            # Repository overview
